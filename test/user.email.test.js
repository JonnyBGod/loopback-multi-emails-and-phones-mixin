...
describe('User with emails', function() {
  this.timeout(10000);
  ...
  var validCredentialsEmailVerified = {
    email: 'foo1@bar.com', password: 'bar1'};
  var validCredentialsEmailVerifiedOverREST = {
    email: 'foo2@bar.com', verified: true};
  ...
  beforeEach(function setupAppAndModels(done) {
    ...
    User.create(validCredentials, function(err, user) {
      ...
      User.create(validCredentialsEmailVerified, function(err, user) {
        ...
        email.verified = true;
        user.emails.set(email.id, email, done);
      });
    });
  });
  ...
  describe('User.create', function() {
    it('Create a new user', function(done) {
      User.create({email: 'f@b.com', password: 'bar'}, function(err, user) {
        assert(!err);
        assert(user.id);
        assert(user.emailAddresses.length);
    ...
    it('Create a new user (email case-sensitive)', function(done) {
      User.create({email: 'F@b.com', password: 'bar'}, function(err, user) {
        ...
        assert(user.id);
        assert(user.emailAddresses[0].email);
    ...
    it('Create a user over REST should remove email verified property', function(done) {
      User.login(validCredentials, function(err, accessToken) {
        if (err) return done(err);
        request(app)
          .post('/test-users/' + accessToken.userId + '/emails?access_token=' + accessToken.id)
  ...
  describe('User.login', function() {
    ...
    it('Login a user providing incomplete credentials', function(done) {
      User.login(incompleteCredentials, function(err, accessToken) {
        assert(err);
        assert.equal(err.code, 'USERNAME_EMAIL_PHONE_REQUIRED');
      ...
      it('Verify a user\'s email address with custom token generator', function(done) {
        User.afterRemote('create', function(ctx, user, next) {
          ...
          var options = {
            ...
            protocol: ctx.req.protocol,
            host: ctx.req.get('host'),
            generateVerificationToken: function(emailAddress, cb) {
              assert(user);
              assert.equal(emailAddress.email, 'bar@bat.com');
              assert(cb);
      ...
      it('Creates a temp accessToken to allow a user to change password', function(done) {
        ...
        User.once('resetPasswordRequest', function(info) {
          ...
          info.accessToken.user(function(err, user) {
            ...
            assert.equal(user.emailAddresses[0].email, email);
  ...
  describe('Verification after updating email', function() {
    ...
    function createOriginalUser(done) {
      ...
      User.create(userData, function(err, instance) {
        ...
        var email = userInstance.emails()[0];
        email.verified = true;
        userInstance.emails.set(email.id, email, done);
      });
    }
  });
