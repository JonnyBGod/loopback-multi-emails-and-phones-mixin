...
describe('User', function() {
  ...
  describe('User.create', function() {
    ...
    it('Create a new user (email case-sensitivity off)', function(done) {
      ...
      User.create({email: 'F@b.com', password: 'bar'}, function(err, user) {
        ...
        assert(user.pk);
        assert.equal(user.emailAddresses[0].email, user.emailAddresses[0].email.toLowerCase());
    ...
    it('Create a new user (email case-sensitive)', function(done) {
      User.create({email: 'F@b.com', password: 'bar'}, function(err, user) {
        ...
        assert(user.pk);
        assert(user.emailAddresses);
        assert.notEqual(user.emailAddresses[0].email, user.emailAddresses[0].email.toLowerCase());
    ...
    it('Create a user over REST should remove email verified property', function(done) {
      request(app)
        .post('/test-users')
  ...
  describe('Access-hook for queries with email NOT case-sensitive', function() {
    ...
    it('Should be able to find lowercase email with mixed-case email query', function(done) {
      ...
      User.find({where: {email: validMixedCaseEmailCredentials.email}}, function(err, result) {
        ...
        assert.equal(result[0].emailAddresses[0].email, validCredentialsEmail);
    ...
    it('Should be able to use query filters (email case-sensitivity off)', function(done) {
      ...
      User.create(insensitiveUser, function(err, user) {
        User.find({where: {email:
          ...
          if (err) done(err);
          assert(result[0], 'The query did not find the user');
          assert.equal(result[0].emailAddresses[0].email, insensitiveUser.email);
          done();
        });
      });
    });
    ...
    it('Should be able to use query filters (email case-sensitivity on)', function(done) {
      ...
      User.create(sensitiveUser, function(err, user) {
        User.find({where: {email:
          ...
          if (err) done(err);
          assert(result[0], 'The query did not find the user');
          assert.equal(result[0].emailAddresses[0].email, sensitiveUser.email);
          done();
        });
      });
    });
    ...
    it('Returns current user when `include` is `USER`', function(done) {
      request(app)
        ...
        .end(function(err, res) {
          ...
          expect(token.user.emailAddresses[0], 'body.user.emailAddresses[0]')
  ...
  describe('User.login requiring email verification', function() {
    ...
    xit('login a user with verified email', function(done) {
    ...
    xit('login a user with verified email - promise variant', function(done) {
    ...
    xit('login a user over REST when email verification is required', function(done) {
      request(app)
        .post('/test-users/login')
        .expect('Content-Type', /json/)
        .expect(200)
  ...
  describe('Identity verification', function() {
    describe('user.verify(verifyOptions, options, cb)', function() {
      ...
      describe('User.getVerifyOptions()', function() {
        ...
        xit('returns same verifyOptions after verify user model', () => {
      ...
      xit('verifies a user\'s email address with custom token generator', function(done) {
        User.afterRemote('create', function(ctx, user, next) {
          assert(user, 'afterRemote should include result');
      ...
      xit('squashes "//" when restApiRoot is "/"', function(done) {
        var emailBody;
        User.afterRemote('create', function(ctx, user, next) {
          assert(user, 'afterRemote should include result');
      ...
      it('removes "verifyOptions.template" from Email payload', function() {
        ...
        verifyOptions.to = user.emailAddresses[0].email;
      ...
      it('verifies that verifyOptions.templateFn receives verifyOptions.verificationToken',
        function() {
          ...
          Object.assign(verifyOptions, {
            ...
            to: user.emailAddresses[0].email,
          });
        ...
        testConfirm(function(result, done) {
          User.confirm(result.uid, result.token, false, function(err) {
            ...
            User.findById(result.uid, function(err, user) {
              if (err) return done(err);
              expect(user.emailAddresses[0]).to.have.property('verificationToken', null);
              done();
            });
          });
        }, done);
  ...
  describe('Password Reset', function() {
    describe('User.resetPassword(options, cb)', function() {
      ...
      xit('Requires email address to reset password', function(done) {
      ...
      xit('Requires email address to reset password - promise variant', function(done) {
        User.resetPassword({ })
          .then(function() {
      ...
      it('Creates a temp accessToken to allow a user to change password', function(done) {
        ...
        User.once('resetPasswordRequest', function(info) {
          ...
          info.accessToken.user(function(err, user) {
            ...
            assert.equal(user.emailAddresses[0].email, options.email);
        ...
        xit('Creates a temp accessToken to allow user in realm to change password', function(done) {
          ...
          User.once('resetPasswordRequest', function(info) {
            ...
            info.accessToken.user(function(err, user) {
              ...
              assert.equal(user.emailAddresses[0].mail, realmUser.email);
  ...
  describe('Verification after updating email', function() {
      ...
      function(done) {
        ...
        async.series([
          function updateUser(next) {
            var email = userInstance.emails()[0];
            email.email = NEW_EMAIL;
            userInstance.emails.set(email.id, email, function(err, info) {
              if (err) return next(err);
          ...
          function findUser(next) {
            User.findById(userInstance.pk, function(err, info) {
              if (err) return next(err);
              assert.equal(info.emailAddresses[0].email, NEW_EMAIL);
              assert.equal(info.emailAddresses[0].verified, false);
              next();
            });
          },
        ], done);
    ...
    xit('leaves verification as is after email update if verification is not required',
      function(done) {
        User.settings.emailVerificationRequired = false;
        async.series([
          function updateUser(next) {
          ...
          function findUser(next) {
            User.findById(userInstance.pk, function(err, info) {
              if (err) return next(err);
              assert.equal(info.emailAddresses[0].email, NEW_EMAIL);
              assert.equal(info.emailAddresses[0].verified, true);
              next();
            });
          },
        ], done);
    ...
    xit('should not set verification to false after something other than email is updated',
      function(done) {
        User.settings.emailVerificationRequired = true;
        async.series([
          function updateUser(next) {
  ...
  describe('password reset with/without email verification', function() {
    xit('allows resetPassword by email if email verification is required and done',
      function(done) {
        User.settings.emailVerificationRequired = true;
